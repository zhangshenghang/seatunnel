...
package org.apache.seatunnel.connectors.seatunnel.common.source.reader.synchronization;
...
public class FutureCompletingBlockingQueue<T> {
    ...
    /** The element queue. */
    @GuardedBy("lock") private final Queue<T> queue;

    /** The per-thread conditions that are waiting on putting elements. */
    @GuardedBy("lock") private final Queue<Condition> notFull;

    /** The per-thread conditions and wakeUp flags. */
    @GuardedBy("lock") private ConditionAndFlag[] putConditionAndFlags;
    ...
    @GuardedBy("lock") private void moveToAvailable() {
    ...
    @GuardedBy("lock") private void moveToUnAvailable() {
    ...
    public T take() throws InterruptedException {
        ...
        while ((next = poll()) == null) {
            ...
            try {
                ...
                throw new SeaTunnelException("exception in queue future completion", e);
            }
        }
        return next;
    }
    ...
    @GuardedBy("lock") private void enqueue(T element) {
    ...
    @GuardedBy("lock") private T dequeue() {
        final int sizeBefore = queue.size();
    ...
    @GuardedBy("lock") private void waitOnPut(int fetcherIndex) throws InterruptedException {
    ...
    @GuardedBy("lock") private void signalNextPutter() {
        if (!notFull.isEmpty()) {
            notFull.poll().signal();
        }
    }

    @GuardedBy("lock") private void maybeCreateCondition(int threadIndex) {
    ...
    @GuardedBy("lock") private boolean getAndResetWakeUpFlag(int threadIndex) {
        maybeCreateCondition(threadIndex);

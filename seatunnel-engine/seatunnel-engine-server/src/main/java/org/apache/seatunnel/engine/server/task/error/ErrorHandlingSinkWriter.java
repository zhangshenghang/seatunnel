...
/**
 * SinkWriter wrapper that adds row-level error handling on top of a delegate writer.
 *
 * <p>This implementation follows the design in the error-handling document: it delegates all
 * methods to the underlying writer, but wraps {@link #write(Object)} to classify row-level errors
 * and route them to {@link ErrorHandler} instead of failing the whole job when configured.
 */
@Slf4j

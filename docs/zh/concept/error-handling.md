...
- 将明显的基础设施问题（例如无法建立连接、OOM 等）视为 **系统级错误**，直接失败作业；
- 尝试将典型的 **数据/约束问题** 识别为行级错误。
...
| `original_data_format` | 字符串    | `JSON`  | 原始数据序列化格式。当前版本内部统一按字符串形式写入错误表，预留该参数用于后续区分 `JSON`/`TEXT`/`BINARY` 等序列化策略。 |

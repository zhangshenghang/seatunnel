...
在 SeaTunnel 中，默认行为为：只要某个 Connector 或 Transform 抛出异常，**整个作业就会失败**。

从本实验性能力开始，用户可以改变这一行为，由引擎 **捕获异常数据，将其路由到错误 Sink，并在条件允许时继续推进作业**。

> **状态：实验性（Experimental）**
>
> 错误处理与行级错误路由默认关闭，配置和语义在后续版本中可能有调整。
...
| 参数                     | 类型     | 默认值   | 说明 / 取值                                                                                 |
|------------------------|--------|--------|------------------------------------------------------------------------------------------|
| `mode`                 | 字符串    | `DISABLE` | 行级错误处理模式：`DISABLE`（关闭）、`LOG`（只记录）、`ROUTE`（记录并路由到错误 Sink）。                         |
| （阈值统计口径）              | -      | -      | 当前版本阈值使用内部计数器：Sink 每次 `write(...)` 计 1；Transform 链中每个 `map(...)`/`flatMap(...)` 调用计 1；同一条 Transform 链上的多个算子共享同一个计数器。 |
| `max_error_ratio`      | 小数     | `0.0`   | 允许的错误比例，0.0–1.0；例如 `0.01` 表示错误记录超过 1% 时失败作业；`0.0` 表示不按比例触发失败。                          |
| `max_error_ratio_min_records` | 整型 | `100`  | `max_error_ratio` 的预热阈值：当总处理记录数小于该值时，不进行比例触发，避免在样本很小时误触发失败。 |
| `max_error_records`    | 长整型    | `0`     | 允许的错误记录总数上限；`0` 表示不按错误条数触发失败。                                                           |
| `queue_capacity`       | 整型     | `10000` | 内部错误队列（缓冲区）容量上限，队列中最多可同时缓存的错误记录数量。                                                     |
| `queue_overflow_policy`| 字符串    | `FAIL`  | 错误队列已满时的策略：`FAIL`（失败作业）、`DROP`（丢弃新错误记录）、`BLOCK`（阻塞生产错误的线程，可能影响吞吐）。                 |
| `include_original_data`| 布尔型    | `false` | 是否在错误记录中包含原始数据内容。                                             |
...
| 参数            | 类型   | 说明                                                                 |
|-----------------|------|--------------------------------------------------------------------|
| `plugin_name`   | 字符串 | 错误 Sink 使用的 Connector 名称，例如 `Jdbc`。                               |
| `error_table`   | 字符串 | （JDBC 专用）错误记录要写入的目标表名，例如 `orders_sink_error_basic`。             |
...
上述行为属于 Connector 级别的当前实现细节，后续会逐步对不同 Sink 的实现进行优化，以降低误伤合法记录的概率并提升可追溯性。

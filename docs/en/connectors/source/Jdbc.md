...
| split.string-strategy                      | String  | No       | -               | Controls how String partition columns are split. `range` uses range-based splitting, `hash` uses hash-based splitting, `none` disables String splitting, and `auto` prefers range splitting then falls back to hash when needed. If not configured, SeaTunnel keeps the legacy behavior for the current splitter implementation.                                                                                                                                                                                                                                                                       |
...
| split.enable-hash-split-for-string-column  | Boolean | No       | -               | Deprecated compatibility option. `true` maps to `split.string-strategy = hash`, `false` maps to `split.string-strategy = none` when `split.string-strategy` is not explicitly configured.                                                                                                                                                                                                                                                                                                                                                                                                                     |
